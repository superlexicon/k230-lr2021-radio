...
int16_t SX127x::begin(uint8_t* chipVersions, uint8_t numVersions, uint8_t syncWord, uint16_t preambleLength) {
  ...
  state = SX127x::setCurrentLimit(0);//60
  RADIOLIB_ASSERT(state);
...
int16_t SX127x::startReceive() {
  return(this->startReceive(0, RADIOLIB_SX127X_RXSINGLE));//RADIOLIB_SX127X_RXCONTINUOUS
}
...
int16_t SX127x::readData(uint8_t* data, size_t len) {
  ...
  printf("SX127x::readData packet_length:%zu\n",length);
  size_t dumpLen = 0;
  if((len != 0) && (len < length)) {
